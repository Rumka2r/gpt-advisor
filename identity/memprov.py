...
def stamp(path, session_id=None, tool=None):
    ...
    # Автор — структурное поле из назначенной launcher'ом идентичности, а не
    # текстовая метка [glm] в теле (вердикт 06.08, пункт г). Факты остаются
    # общими для чтения; ownership и visibility — разные вещи.
    author = None
    try:
        import continuity_identity as _ident
        author = _ident.current_agent_id()
    except Exception:
        pass
