...
def summarize_one(path, project, use_llm=True):
    ...
    tools, files = T.tool_summary(info["actions"])
    # Идентичность — из ПРИВЯЗКИ сессии в реестре (кто вёл сессию), а не из env
    # суммаризатора: догоняющий воркер чужого агента не должен присваивать
    # чужую сессию себе. Поле `agent:` ниже — «claude/codex», это другое.
    identity_fm = []
    try:
        import continuity_identity as ident
        identity_fm = ident.summary_identity_fm(info["session_id"])
    except Exception:
        identity_fm = ["identity_schema: 1", "agent_id: legacy-unowned"]
    fm = [
        "---",
