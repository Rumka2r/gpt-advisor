...
def main():
    ...
    try:
        ...
        # Разграничение инстансов (вердикт 06.08): авто-подхват работает в
        # scope=mine — свои сессии + общий канон памяти; сессии ДРУГИХ агентов
        # не подмешиваются никогда (видимость ≠ автоматическая инъекция).
        # Legacy-сессии (до разграничения) остаются, но с пометкой владельца.
        # Отказ реестра память не роняет: работаем без фильтра, как раньше.
        try:
            import continuity_identity as ident
            hits, _ = ident.apply_scope(hits, scope="mine")
        except Exception:
            pass
        ...
        for h in fresh:
            ...
            note = (" " + h["owner_note"]) if h.get("owner_note") else ""
            lines.append("— %s · %s (%s)%s" % (h["when"], h["title"][:90], what, note))
            if h.get("frag"):
